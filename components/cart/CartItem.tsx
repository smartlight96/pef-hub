...
export default function CartDrawer({ isOpen, onClose }: CartDrawerProps) {
  const { 
    ...
    decreaseQuantity,
    removeFromCart,
    clearCart  // ← ADD THIS - Clear cart function
  } = useCart();
        ...
        <div className="flex-1 overflow-y-auto px-4 py-3 space-y-2 max-h-[calc(100vh-200px)] custom-scrollbar">
          {cart.length === 0 ? (
            ...
            <>
              ...
              <button
                onClick={handleClearCart}
                className="w-full mt-2 py-2 text-xs text-zinc-500 hover:text-red-400 transition-colors border border-zinc-800/30 rounded-lg hover:border-red-500/20 hover:bg-red-500/5"
              >
                Clear all items
              </button>
            </>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 backdrop-blur-sm">
          <div className="bg-zinc-900 rounded-2xl border border-zinc-800 p-6 max-w-sm w-full mx-4 shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="text-lg font-bold text-white">Clear Cart?</h3>
            </div>
